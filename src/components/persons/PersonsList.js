...
function PersonsList(props) {
  ...
  const getUsers = (filters) => {
    if (filters != "") {
      props.getAllUsers(filters).then(() => {
        ...
        if (props.getAllUsersError) {
          alert("Could not get persons due to network error")
        }
      });
    } else {
  ...
  const columns = [
    { header: "Photo", body: customBodyTemplate, style: { width: '3%' }, bodyStyle: { textAlign: 'center' }, alignHeader: 'center' },
    ...
    // { field: "mobileNumber", header: "Mobile Number", style: { width: '8%' }, bodyStyle: { textAlign: 'center' }, alignHeader: 'center' },
    { field: "city", header: "City", style: { width: '10%' }, bodyStyle: { textAlign: 'center' }, sortable: true, alignHeader: 'center' },
    { field: "districtName", header: "District", style: { width: '10%' }, bodyStyle: { textAlign: 'center' }, alignHeader: 'center' },
    { field: "kycStatus", header: "KYC Status", body: customBodyTemplate, bodyStyle: { textAlign: 'center' }, style: { width: '6%' }, sortable: true, alignHeader: 'center' },
    { field: "vehiclesCount", header: "Type", body: customBodyTemplate, bodyStyle: { textAlign: 'center' }, style: { width: '6%' }, sortable: true, alignHeader: 'center' },
    { header: "Actions", body: customBodyTemplate, bodyStyle: { textAlign: 'center' }, style: { width: '3%' }, alignHeader: 'center' }
  ];
        ...
        <div className="datatable-templating-demo">
          ...
          <div>
            {displayResponsive === true ?
              <PersonDetails
                data={personData}
                modelVisible={displayResponsive}
                hideModal={() => onHide('displayResponsive')}
              /> : null}
            <Dialog
...
const mapStateToProps = state => {
  return {
    ...
    getAllUsersError: state.getAllUsers.error
  };
};
